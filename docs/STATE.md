...
- [Next session pickup](#next-session-pickup)
  - [Live target: wfmctrading-prod](#live-target-wfmctrading-prod)
...
### Live target: wfmctrading-prod

Real production VM this repo is actively hardening: `wfmctrading-prod` (5.223.44.144, SSH alias `wfmc-prod`, Hetzner/Singapore, sudo user `willard`). App is a marketing site + ERP — not latency-sensitive; users are in the Philippines. Treat script changes as production-affecting, not theoretical.
...
- **The AIDE bug (found + fixed this session, `997adfc`):** `base-server.sh` installed `aide` but never `aide-common` — the package that actually ships `/etc/aide/aide.conf` and the `aideinit` script. Both `aideinit` and the `aide --init` fallback failed silently (stderr suppressed), and the script printed `pass "AIDE database initialized"` **unconditionally**, regardless of whether a database was ever produced. Confirmed live on `wfmctrading-prod`: `/var/lib/aide/` didn't exist at all despite every prior run claiming success. Fix installs `aide-common` and makes the success message conditional on the database file actually existing, `warn`-ing otherwise. **Implication:** any server previously hardened with a pre-`997adfc` version of this script likely has the same silent gap — worth checking (`ls -la /var/lib/aide/`) on any other box this repo has touched.
