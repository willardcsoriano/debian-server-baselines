...
Project-specific drift runbook for `base.sh`. Run this whenever you return to the repo after an absence, or when asked to touch any install section. It covers the components most likely to have changed: Docker (lives in **two** role scripts now — see Section 1), the Bitwarden Secrets Manager CLI (also two role scripts — see Section 2), Netdata, Lynis, and the Debian target version. For each one, fetch the canonical URL, compare against the current script, and report any discrepancies before making changes. General drift methodology lives in `DRIFT.md`; this file is the executable checklist for this repo specifically.

## Table of Contents

- [Overview](#overview)
- [How to invoke](#how-to-invoke)
- [1. Docker (prod.sh + dev.sh, both section 1) — High risk](#1-docker-prodsh-devsh-both-section-1-high-risk)
- [2. Bitwarden Secrets Manager CLI (prod.sh section 2/2 + dev.sh section 7/8) — Medium risk](#2-bitwarden-secrets-manager-cli-prodsh-section-22-devsh-section-78-medium-risk)
- [3. Netdata (base.sh, section 11) — Medium risk](#3-netdata-basesh-section-11-medium-risk)
- [4. Lynis (base.sh, section 18) — Medium risk](#4-lynis-basesh-section-18-medium-risk)
- [5. Debian target version — Medium risk](#5-debian-target-version-medium-risk)
- [6. Antigravity CLI (dev.sh, section 8) — High risk](#6-antigravity-cli-devsh-section-8-high-risk)
...
## 1. Docker (prod.sh + dev.sh, both section 1) — High risk

**Why:** Docker changes its apt repo format, package names, and rootless install procedure roughly annually. The rootless Docker install is **duplicated** between `prod.sh` (section 1/2) and `dev.sh` (section 1/8) — the repo's curl-per-script install model precludes a shared `lib/`. Both copies must be edited in lockstep.

**Fetch:**
- https://docs.docker.com/engine/install/debian/
- https://docs.docker.com/engine/security/rootless/

**Check against both `prod.sh` section 1 and `dev.sh` section 1:**
...
- **Drift between the two copies.** `diff <(sed -n '/^# DRIFT: apt repo format/,/^pass "Docker/p' scripts/prod.sh) <(sed -n '/^# DRIFT: apt repo format/,/^pass "Docker/p' scripts/dev.sh)` should be empty except the `NOTE:` line's cross-referenced filename (each file names the *other* one — expected, not drift). Anchor on the literal `# DRIFT: apt repo format` / `pass "Docker` text, not a bare `/^# DRIFT:/,/^pass /p` — both scripts now contain multiple `# DRIFT:`/`pass` pairs, and a bare pattern re-triggers on every one instead of isolating Docker's block, producing false-positive diffs.
...
## 2. Bitwarden Secrets Manager CLI (prod.sh section 2/2 + dev.sh section 7/8) — Medium risk

**Why:** `bws` ships as prebuilt zips on GitHub releases under `bitwarden/sdk-sm`, sha256-verified against a sibling checksums file. Tag format, asset naming, or checksum filename could shift. The install logic is **duplicated** between `prod.sh` (section 2/2) and `dev.sh` (section 7/8) — same curl-per-script constraint as Docker. The surrounding scaffolding differs (prod.sh sets up `~/.local/bin`/PATH and `unzip` inline since it lacks dev.sh's earlier sections that already do this), but the `# DRIFT:` comment and the `if command -v bws ... fi` install logic must stay identical.

**Fetch:**
- https://bitwarden.com/help/secrets-manager-cli/
- https://github.com/bitwarden/sdk-sm/releases

**Check against both `prod.sh` section 2/2 and `dev.sh` section 7/8:**
...
- **Drift in the DRIFT comment.** `diff <(sed -n '/^# DRIFT: bws is published/,/^# Last verified:/p' scripts/prod.sh) <(sed -n '/^# DRIFT: bws is published/,/^# Last verified:/p' scripts/dev.sh)` should be empty except the `NOTE:` line's cross-referenced section number (`2/2` vs `7/8` is expected, not drift).
- **Drift in the install logic.** `diff <(sed -n '/^if command -v bws/,/^fi$/p' scripts/prod.sh) <(sed -n '/^if command -v bws/,/^fi$/p' scripts/dev.sh)` should be byte-identical (empty diff). If it isn't, someone updated one copy's install logic and forgot the other.
...
## 3. Netdata (base.sh, section 11) — Medium risk
...
**Check against `base.sh` section 11:**
...
## 4. Lynis (base.sh, section 18) — Medium risk
...
**Check against `base.sh` section 18:**
...
## 6. Antigravity CLI (dev.sh, section 8) — High risk
...
**Check against `dev.sh` section 8:**
...
**Report:** any installer-URL change, binary rename, install-path move, new Node dependency, or new pinning option. If anything changed, propose the fix to `dev.sh` section 8.
...
```bash
# Syntax
bash -n scripts/base.sh scripts/prod.sh scripts/dev.sh scripts/syslog.sh scripts/wireguard.sh

# Static analysis
shellcheck scripts/base.sh scripts/prod.sh scripts/dev.sh scripts/syslog.sh scripts/wireguard.sh

# Base script section count (should still be 20)
grep -c 'section "' scripts/base.sh
grep -oP '\d+(?=/)' scripts/base.sh | sort -n | uniq | tail -1   # highest N in N/20

# prod.sh section count (should still be 2)
grep -c 'section "' scripts/prod.sh
grep -oP '\d+(?=/2)' scripts/prod.sh | sort -n | uniq | tail -1             # highest N in N/2

# dev.sh section count (should still be 8)
grep -c 'section "' scripts/dev.sh
grep -oP '\d+(?=/8)' scripts/dev.sh | sort -n | uniq | tail -1              # highest N in N/8

# Summary block matches section count
grep -c '✓\|✗\|–' scripts/base.sh | tail -1
```
...
If anything is CHANGED, propose the diff to the relevant script(s) before making it. Docker and bws changes must land in **both** `prod.sh` and `dev.sh` in the same commit.
